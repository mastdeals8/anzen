/*
  # Fix Sales Invoice COGS Calculation - Correct Column Name
  
  1. Issue
    - The trigger was referencing b.purchase_price which doesn't exist
    - The batches table uses final_landed_cost or cost_per_unit
  
  2. Fix
    - Use COALESCE(b.final_landed_cost, b.cost_per_unit, 0) for proper cost
    - This ensures we use the most accurate cost available
*/
...
BEGIN
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.journal_entry_id IS NULL AND NEW.payment_status IN ('pending', 'partial', 'paid')) THEN
    ...
    IF v_cogs_account_id IS NOT NULL AND v_inventory_account_id IS NOT NULL THEN
      -- Calculate total COGS - FIX: Use correct column names
      FOR v_item_record IN
        SELECT 
          sii.quantity,
          COALESCE(b.final_landed_cost, b.cost_per_unit, 0) as cost_per_unit,
          (sii.quantity * COALESCE(b.final_landed_cost, b.cost_per_unit, 0)) as total_cost
