/*
  # Fix Sales Invoice Trigger - Use Correct Journal Entry Columns
  
  1. Problem
    - Trigger uses reference_type but column is source_module
    - Trigger uses created_by in VALUES but should use posted_by
    - This causes error: column "reference_type" does not exist
  
  2. Solution
    - Use source_module instead of reference_type
    - Use posted_by in VALUES instead of created_by
    - Match actual journal_entries table schema
  
  3. Correct Schema
    - source_module (not reference_type)
    - reference_id
    - reference_number
    - posted_by (for the user who posted)
    - created_by (exists in table but set automatically)
*/
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.journal_entry_id IS NULL AND NEW.payment_status IN ('pending', 'partial', 'paid')) THEN
    ...
    -- Create journal entry for revenue (using correct column names)
    INSERT INTO journal_entries (
      ...
      'Sales Invoice: ' || NEW.invoice_number, NEW.total_amount, NEW.total_amount, true, NEW.created_by
    ...
    IF v_cogs_account_id IS NOT NULL AND v_inventory_account_id IS NOT NULL THEN
      ...
      IF v_total_cogs > 0 THEN
        ...
        -- Create journal entry for COGS (using correct column names)
        INSERT INTO journal_entries (
          entry_number, entry_date, source_module, reference_id, reference_number,
          description, total_debit, total_credit, is_posted, posted_by
        ) VALUES (
