/*
  # Fix Journal Entry Number Generation - Remove Sequence
  
  1. Problem
    - Trigger uses NEXTVAL('journal_entry_number_seq') but sequence doesn't exist
    - Error: relation "journal_entry_number_seq" does not exist
    - Prevents invoice creation
  
  2. Solution
    - Use COUNT method like all other triggers in the system
    - Pattern: COUNT(*) + 1 FROM journal_entries WHERE entry_number LIKE pattern
    - This is consistent with existing codebase
  
  3. Changes
    - Replace NEXTVAL with COUNT-based numbering
    - Use 'YYMM' format like other triggers
*/
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.journal_entry_id IS NULL AND NEW.payment_status IN ('pending', 'partial', 'paid')) THEN
    ...
    -- Generate journal entry number using COUNT method (consistent with other triggers)
    ...
    IF v_cogs_account_id IS NOT NULL AND v_inventory_account_id IS NOT NULL THEN
      ...
      IF v_total_cogs > 0 THEN
        -- Generate new journal entry number for COGS using COUNT method
        v_je_number := 'JE' || TO_CHAR(NEW.invoice_date, 'YYMM') || '-' || LPAD((
          SELECT COUNT(*) + 1 FROM journal_entries WHERE entry_number LIKE 'JE' || TO_CHAR(NEW.invoice_date, 'YYMM') || '%'
        )::TEXT, 4, '0');
