/*
  # Fix Sales Invoice Journal Entry Balance Error
  
  1. Problem
    - Journal entry header sets total_credit = NEW.total_amount
    - But actual credit lines = v_sales_amount + tax_amount
    - These don't match when there's a discount, causing constraint violation
    - The chk_journal_entry_balanced constraint requires total_debit = total_credit
  
  2. Root Cause
    - v_sales_amount = subtotal - discount
    - Credits = (subtotal - discount) + tax
    - But header says total_credit = total_amount (which includes discount)
    - This creates an imbalance
  
  3. Solution
    - Calculate correct total_credit as v_sales_amount + tax_amount
    - Ensure header totals match the actual line totals
*/
